...
if ~exist(fullMatFileName, 'file')
  ...
  valid_data = tempdata.Validation_process_data;
end
standard_Train_data=[];standard_Test_data=[];standard_Valid_data=[];
[m,n]=size(train_data);
...
[m2,n2]=size(valid_data);
valid_label=valid_data(:,n2);
standard_Valid_data =  fn_TestFeatureStandardize(valid_data(:,1:n2-1), mean_Train_data, std_Train_data);
standard_Valid_data = [standard_Valid_data,valid_label];
...
save(fulldestination, 'standard_Train_data','standard_Valid_data','standard_Test_data');
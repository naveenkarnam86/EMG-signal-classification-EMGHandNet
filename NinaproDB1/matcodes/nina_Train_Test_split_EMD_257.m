...
[Train_raw_data Test_raw_data] = fn_final_train_test_split_257(sub_data,Trial_length,n_class);
 train_process_sub_wise_data= fn_raw_trial_EMD(Train_raw_data,Trial_length);
 train_process_data=[train_process_data;train_process_sub_wise_data];
 
destdirectory = '/media/naveen/nav/mat_codes/nina_DB1_codes/nina_prep_python/prep_EMD_wave/sub_wise_process_TT/';
...
test_process_sub_wise_data= fn_raw_trial_EMD(Test_raw_data,Trial_length);
...
destdirectory = '/media/naveen/nav/mat_codes/nina_DB1_codes/prep_EMD_wave/Total_process_TT/';
mkdir(destdirectory);   %create the directory
file1 = sprintf('nina_pro_naveen_DB1_EMD_wave_train.csv');
fulldestination = fullfile(destdirectory, file1);%name file relative to that directory
csvwrite(fulldestination, train_process_data); 
file2 = sprintf('nina_pro_naveen_DB1_EMD_wave_test.csv');
...
file3 = sprintf('nina_pro_naveen_DB1_EMD_wave_train_mat.mat');
fulldestination = fullfile(destdirectory, file3);
save(fulldestination, 'train_process_data');
file4 = sprintf('nina_pro_naveen_DB1_EMD_wave_test_mat.mat');

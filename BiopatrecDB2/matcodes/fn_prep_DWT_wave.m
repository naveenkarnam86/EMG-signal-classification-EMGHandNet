function [DWT_data]=fn_prep_DWT_wave(trial_data)
preprocess_data=[];
DWT_data=[];
[m,n]=size(trial_data);
...
% n_IMF = 4;
% alpha = 500;tau=0; K =4; DC = 0; init=1; tol= 1e-7;
...
for nx =1:Npat
   ...
   for ch = 1:n
       ...
       [DWT_seg,l] = wavedec(segment,4,'sym8');
%        approx = appcoef(DWT_seg,l,'sym2');
%        [cd1,cd2,cd3,cd4] = detcoef(DWT_seg,l,[1 2 3 4]);
        DWT_10ch = [DWT_10ch DWT_seg];
   end
        DWT_data = [DWT_data; DWT_10ch];
end
       %         segment= trial_data((nx-1)*segment_length+1:nx*segment_length,ch);

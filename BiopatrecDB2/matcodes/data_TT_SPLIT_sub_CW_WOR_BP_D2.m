...
for sub=1:L
 ...
 train_process_sub_wise_data= fn_raw_trial_CW_d2_over(train_raw_data,Trial_length);
 train_process_data=[train_process_data;train_process_sub_wise_data];
 test_process_sub_wise_data= fn_raw_trial_CW_d2_over(test_raw_data,Trial_length);
 test_process_data=[test_process_data;test_process_sub_wise_data];
 
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/prep_CW_WOR_BP_D2/sub_wise_process_TT/';
...
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/prep_CW_WOR_BP_D2/Total_process_TT/';
mkdir(destdirectory);   %create the directory
file1 = sprintf('Biopatrec_CW_WOR_BP_D2_train.csv');
fulldestination = fullfile(destdirectory, file1);%name file relative to that directory
csvwrite(fulldestination, train_process_data); 
file2 = sprintf('Biopatrec_CW_WOR_BP_D2_test.csv');

...
% valid_process_data = [];
n_class=26;Trial_length=12000;
myfolder = dir('/media/naveen/nav/mat_codes/BioPatRec/sub_wise_WR/S*');
...
for sub=1:L
 ...
 train_process_sub_wise_data= fn_raw_trial_d2_over(train_raw_data,Trial_length);
 train_process_data=[train_process_data;train_process_sub_wise_data];
 test_process_sub_wise_data= fn_raw_trial_d2_over(test_raw_data,Trial_length);
 test_process_data=[test_process_data;test_process_sub_wise_data];
 
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/prep_BP_D2_WR/sub_wise_process_TT/';
...
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/prep_BP_D2_WR/Total_process_TT/';
mkdir(destdirectory);   %create the directory
file1 = sprintf('Biopatrec_26class_17sub_train_WR_BP_D2.csv');
fulldestination = fullfile(destdirectory, file1);%name file relative to that directory
csvwrite(fulldestination, train_process_data); 
file2 = sprintf('Biopatrec_26class_17sub_test_WR_BP_D2.csv');

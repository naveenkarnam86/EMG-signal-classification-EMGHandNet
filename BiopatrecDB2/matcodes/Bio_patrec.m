...
%   for k=0:m1:m2
    for j=0:Trial_length_WR:m2-Trial_length_WR
      class_vec=class_label*ones(Trial_length,1);
      trial_data=data3(j+1:j+Trial_length,:);
      trial_cl=[trial_data class_vec];
      sub_wise_data=[sub_wise_data;trial_cl];
      if(mod(size(sub_wise_data,1),18000)==0)   
        class_label=class_label+1; 
      end
  end
      
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/sub_wise';

...
for sub=1:L
 ...
 sub_seg_data=fn_raw_seg(sub_data,Trial_length);
  [train_sub_raw_data test_sub_raw_data]=fn_final_train_test_split_13(sub_seg_data,new_Trial_length,n_class);
 train_raw_data=[train_raw_data;train_sub_raw_data];
 test_raw_data=[test_raw_data;test_sub_raw_data];
 
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/prep_seg_CW_WOR_BP_D2/sub_wise_process_TT/';
...
disp(sub);
 ...
 train_process_sub_wise_data= fn_BP_D2_over(train_raw_data,new_Trial_length);
 train_process_data=[train_process_data;train_process_sub_wise_data];
 test_process_sub_wise_data= fn_BP_D2_over(test_raw_data,new_Trial_length);
 test_process_data=[test_process_data;test_process_sub_wise_data];
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/prep_seg_CW_WOR_BP_D2/Total_process_TT/';
mkdir(destdirectory);   %create the directory
file1 = sprintf('Biopatrec_seg_CW_WOR_BP_D2_train.csv');
fulldestination = fullfile(destdirectory, file1);%name file relative to that directory
csvwrite(fulldestination, train_process_data); 
file2 = sprintf('Biopatrec_seg_CW_WOR_BP_D2_test.csv');

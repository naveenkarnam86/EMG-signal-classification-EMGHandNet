...
fullMatFileName = fullfile(destdirectory,  'Biopatrec_26c_17s_LP_wave_train_mat.mat')
...
fullMatFileName = fullfile(destdirectory,  'Biopatrec_26c_17s_LP_wave_test_mat.mat')
if ~exist(fullMatFileName, 'file')
  message = sprintf('%s does not exist', fullMatFileName);
  uiwait(warndlg(message));
else
...
destdirectory = '/media/naveen/nav/mat_codes/BioPatRec/feat';
mkdir(destdirectory);   %create the director
file = sprintf('BPR_DB2_sEMG_feat_data.mat');

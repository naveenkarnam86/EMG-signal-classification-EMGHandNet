...
for sub=1:L
 ...
 train_process_sub_wise_data= fn_raw_trial_seg_MVMD(train_raw_data,Trial_length);
%  train_process_data=[train_process_data;train_process_sub_wise_data];
 
destdirectory = '/media/naveen/nav/mat_codes/nina_DB4_codes/prep_seg_LP_MVMD_wave/sub_wise_process_TT/';
...
train_process_sub_wise_data=[];
 test_process_sub_wise_data= fn_raw_trial_seg_MVMD(test_raw_data,Trial_length);
...
destdirectory = '/media/naveen/nav/mat_codes/nina_DB4_codes/prep_seg_LP_MVMD_wave/Total_process_TT/';
mkdir(destdirectory);   %create the directory
file1 = sprintf('nina_pro_naveen_DB4_seg_MVMD_wave_train.csv');
fulldestination = fullfile(destdirectory, file1);%name file relative to that directory
csvwrite(fulldestination, train_process_data); 
file2 = sprintf('nina_pro_naveen_DB4_seg_MVMD_wave_test.csv');
...
file3 = sprintf('nina_pro_naveen_DB4_seg_MVMD_wave_train_mat.mat');
fulldestination = fullfile(destdirectory, file3);
save(fulldestination, 'train_process_data');
file4 = sprintf('nina_pro_naveen_DB4_seg_MVMD_wave_test_mat.mat');

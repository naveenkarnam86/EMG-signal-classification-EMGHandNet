function [preprocess_data]=fn_prep_notch_nb(trial_data)
preprocess_data=[];
[m,n]=size(trial_data);
...
% data_filtered=filtfilt(b,a,double(emg_notch));

[b1, a1] = butter(filterOrder2,[fL fH]/(fs/2)); % Generate filter classoefficlassients
data_filtered = filtfilt(b1, a1, double(emg_notch));
%%%%%%%%%%%%%%%%%%%%%%%
...
% data_denoised = wdenoise(data_filtered,L,'Wavelet','sym8',...
...
preprocess_data = abs(data_filtered);
% % Normalize the data

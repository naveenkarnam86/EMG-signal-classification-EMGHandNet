function  [Train_all valid_all Test_all] =fn_final_train_test_split_E123(rawdata,Trial_length)
Train_all=[];
valid_all=[];
Test_all=[];
% step = Trial_length;
    for i=1:1:52    
        classwisedata=rawdata(rawdata(:,13)==i,:);
...
%         Trial = fn_trial_wise_data_split_E123(classwisedata_TS)
   ...
   Train_all = Train_all(:, 1:13);
   valid_all =valid_all(:, 1:13);
   Test_all = Test_all(:, 1:13);
return 
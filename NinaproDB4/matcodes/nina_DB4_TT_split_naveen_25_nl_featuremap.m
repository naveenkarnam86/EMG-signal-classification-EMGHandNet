...
[Train_raw_data Test_raw_data] = fn_final_train_test_split_25_featmap(sub_data,Trial_length,n_class);
...
Train_process_sub_wise_data = fn_raw_trial_nl(Train_raw_data,Trial_length);
Train_process_data = [Train_process_data;Train_process_sub_wise_data];
Test_process_sub_wise_data = fn_raw_trial_nl(Test_raw_data,Trial_length);
...
destdirectory = '/media/naveen/nav/mat_codes/nina_DB4_codes/naveen_prep_nlw/sub_wise_process_TT/';
...
destdirectory = '/media/naveen/nav/mat_codes/nina_DB4_codes/naveen_prep_nlw/Total_process_TT/';
mkdir(destdirectory);   %create the director
file1 = sprintf('nina_naveen_nlw_DB4_Train.csv');
fulldestination = fullfile(destdirectory, file1);%name file relative to that directory
% save(fulldestination, 'Train_process_data','Test_process_data');
csvwrite(fulldestination,Train_process_data);
file2 = sprintf('nina_naveen_nlw_DB4_Test.csv');
fulldestination = fullfile(destdirectory, file2);

...
fullMatFileName = fullfile(destdirectory,  'nina_naveen_nlw_DB4_Train_mat.mat')
...
fullMatFileName = fullfile(destdirectory,  'nina_naveen_nlw_DB4_Test_mat.mat')
if ~exist(fullMatFileName, 'file')
  message = sprintf('%s does not exist', fullMatFileName);
  uiwait(warndlg(message));
else
...
destdirectory = '/media/naveen/nav/mat_codes/nina_DB4_codes/feat';
mkdir(destdirectory);   %create the director
file = sprintf('nina_DB4_sEMG_feat_data.mat');

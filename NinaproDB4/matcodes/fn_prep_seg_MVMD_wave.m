function [mv_modes_data]=fn_prep_seg_MVMD_wave(trial_data)
preprocess_data=[];
mv_modes_data=[];
[m,n]=size(trial_data);
...
%     parfor ch = 1:n
        ...
        [mv_modes, u_hat, omega] = MVMD_ver1(segment, alpha, tau, K, DC, init, tol);
        mv_modes_permute =  permute(mv_modes,[2 1 3]);
        [m1,n1,p1] = size(mv_modes_permute);
        MVMD_seg = reshape(mv_modes_permute,[m1,n1*p1]);
        mv_modes_data =[mv_modes_data; MVMD_seg];
end

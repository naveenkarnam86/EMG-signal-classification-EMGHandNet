function [process_data]=fn_raw_trial_nb_FT_whole(raw_data,Trail_length)
...
overlap=50;
   for i=0:step:m-step
         norm_data=fn_prep_notch_bp(raw_data(i+1:i+step,1:n-1));
         class_label=raw_data(i+1:i+step,n);
         filter_data=[norm_data,class_label];
         data_downsampled = downsample(filter_data,2);

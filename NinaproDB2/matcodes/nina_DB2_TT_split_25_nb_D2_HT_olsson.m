...
Train_process_sub_wise_data = fn_raw_trial_nb_FT_whole(Train_raw_data,Trial_length);
Train_process_data = [Train_process_data;Train_process_sub_wise_data];
Test_process_sub_wise_data = fn_raw_trial_nb_FT_whole(Test_raw_data,Trial_length);
Test_process_data = [Test_process_data;Test_process_sub_wise_data];
new_Trial_length = 5000;
...
destdirectory = '/media/naveen/nav/mat_codes/nina_DB2_codes/prep_nb_D2_HT_olsson/sub_wise_process_TT/';
...
destdirectory = '/media/naveen/nav/mat_codes/nina_DB2_codes/prep_nb_D2_HT_olsson/Total_process_TT/';
mkdir(destdirectory);   %create the director
file1 = sprintf('nina_pro_nb_D2_HT_DB2_Train.csv');
fulldestination = fullfile(destdirectory, file1);%name file relative to that directory
% save(fulldestination, 'Train_process_data','Test_process_data');
csvwrite(fulldestination,Train_process_data);
file2 = sprintf('nina_pro_nb_D2_HT_DB2_Test.csv');
fulldestination = fullfile(destdirectory, file2);

...
[m,n]=size(train_data);
idx=randperm(m);
train_data_feat=train_data(idx(1:m),:);

% [m1,n1]=size(test_data);
% idx=randperm(m1);
% test_data_feat=test_data(idx(1:m1),:);
...
disp(acc1);
Test_save = [Y_Test pred1];
n_class = 49;
n_vec = linspace(1,n_class,n_class);
n_vec_cat = linspace(0,n_class-1,n_class);
Test_save_categorical= changem(Test_save,n_vec_cat,n_vec);
[m3,n3] = size(Test_save_categorical);
Energy_DB2_test_data = Test_save_categorical(1:m3-1,:);
destdirectory = '/media/naveen/nav/mat_codes/nina_DB2_codes/nina_prep_naveen/energy/';
mkdir(destdirectory);   %create the director
file1 = sprintf('energy_DB2_test.csv');
...
csvwrite(fulldestination,Energy_DB2_test_data);